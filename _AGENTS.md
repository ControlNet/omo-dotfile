...
- For non-trivial implementation work, load the beads skill and track execution in Beads (bd): create or continue an issue, keep current progress + next step in the issue, and close/update it with verification notes before ending the session. Use `bd init --skip-hooks` if the beads is not initialized in the current repository.
