...
SearchResponses = dict[str, SearchResponse]
ReasoningResponse = dict[str, object]


def _as_row(value: object) -> Row:
    """Normalize a dict-like value into a string-only row."""
    if not isinstance(value, dict):
        return {}
    return {str(key): str(item) for key, item in value.items()}


def _as_rows(value: object) -> Rows:
    """Normalize a list-like value into CSV-style rows."""
    if not isinstance(value, list):
        return []
    rows: Rows = []
    for item in value:
        rows.append(_as_row(item))
    return rows
...
class DesignSystemGenerator:
    ...
    def __init__(self):
        self.reasoning_data: Rows
        self.reasoning_data = self._load_reasoning()

    def _load_reasoning(self) -> Rows:
        """Load reasoning rules from CSV."""
        filepath = DATA_DIR / REASONING_FILE
        if not filepath.exists():
            return []
        with open(filepath, "r", encoding="utf-8") as f:
            return cast(Rows, list(csv.DictReader(f)))

    def _multi_domain_search(
        self, query: str, style_priority: list[str] | None = None
    ) -> SearchResponses:
        ...
        for domain, config in SEARCH_CONFIG.items():
            if domain == "style" and style_priority:
                ...
                results[domain] = cast(
                    SearchResponse,
                    search(combined_query, domain, config["max_results"]),
                )
            else:
                results[domain] = cast(
                    SearchResponse, search(query, domain, config["max_results"])
                )
        return results

    def _find_reasoning_rule(self, category: str) -> Row:
    ...
    def _apply_reasoning(
        self, category: str, search_results: SearchResponses
    ) -> ReasoningResponse:
        ...
        try:
            loaded_rules = json.loads(rule.get("Decision_Rules", "{}"))
            if isinstance(loaded_rules, dict):
                decision_rules = {
                    str(key): value for key, value in loaded_rules.items()
                }
    ...
    def _select_best_match(self, results: Rows, priority_keywords: list[str]) -> Row:
    ...
    def _extract_results(self, search_result: SearchResponse) -> Rows:
        """Extract results list from search result dict."""
        raw_results = search_result.get("results", [])
        return _as_rows(raw_results)

    def generate(self, query: str, project_name: str | None = None) -> SearchResponse:
        """Generate complete design system recommendation."""
        # Step 1: First search product to get category
        product_result = cast(SearchResponse, search(query, "product", 1))
        product_results = self._extract_results(product_result)
        category = "General"
        if product_results:
        ...
        style_priority = _as_string_list(reasoning.get("style_priority", []))
        ...
        return {
            ...
            "anti_patterns": _as_string(reasoning.get("anti_patterns", "")),
            "decision_rules": reasoning.get("decision_rules", {}),
            "severity": _as_string(reasoning.get("severity", "MEDIUM"), "MEDIUM"),
        }
...
def format_ascii_box(design_system: SearchResponse) -> str:
    """Format design system as ASCII box with emojis (MCP-style)."""
    project = _as_string(design_system.get("project_name", "PROJECT"), "PROJECT")
    pattern = _as_row(design_system.get("pattern", {}))
    style = _as_row(design_system.get("style", {}))
    colors = _as_row(design_system.get("colors", {}))
    typography = _as_row(design_system.get("typography", {}))
    effects = _as_string(design_system.get("key_effects", ""))
    anti_patterns = _as_string(design_system.get("anti_patterns", ""))
...
def format_markdown(design_system: SearchResponse) -> str:
...
def generate_design_system(
    ...
    Args:
        ...
        persist: If True, save design system to <output-dir>/design-system/<project-slug>/
...
def persist_design_system(
    ...
    project_name = _as_string(design_system.get("project_name", "default"), "default")
...
def format_master_md(design_system: SearchResponse) -> str:
...
def format_page_override_md(
    design_system: SearchResponse, page_name: str, page_query: str | None = None
) -> str:
    ...
    lines.append("")
    layout = _as_row(page_overrides.get("layout", {}))
    if layout:
    ...
    lines.append("")
    spacing = _as_row(page_overrides.get("spacing", {}))
    if spacing:
    ...
    lines.append("")
    typography = _as_row(page_overrides.get("typography", {}))
    if typography:
    ...
    lines.append("")
    colors = _as_row(page_overrides.get("colors", {}))
    if colors:
    ...
    lines.append("")
    components = _as_string_list(page_overrides.get("components", []))
    if components:
    ...
    lines.append("")
    unique_components = _as_string_list(page_overrides.get("unique_components", []))
    ...
    lines.append("")
    recommendations = _as_string_list(page_overrides.get("recommendations", []))
    if recommendations:
...
def _generate_intelligent_overrides(
    page_name: str, page_query: str | None, design_system: SearchResponse
) -> SearchResponse:
    """
    ...
    style_search = cast(
        SearchResponse, search(combined_context, "style", max_results=1)
    )
    ux_search = cast(SearchResponse, search(combined_context, "ux", max_results=3))
    landing_search = cast(
        SearchResponse, search(combined_context, "landing", max_results=1)
    )

    # Extract results from search response
    style_results = _as_rows(style_search.get("results", []))
    ux_results = _as_rows(ux_search.get("results", []))
    landing_results = _as_rows(landing_search.get("results", []))
...
def _detect_page_type(context: str, style_results: Rows) -> str:
